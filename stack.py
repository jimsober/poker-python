#stack.py
#
# 5-card draw poker with self-selected starting hand
# Royal Flush    250
# Straight Flush  50
# 4 of a Kind     25
# Full House       9
...
def select(deck):
    hand = []
    print "Select up to 5 cards for your hand."
    selecting = True
    selected_count = 0
    while selecting and selected_count < 5:
        input_err = True
        while input_err:
            selected = raw_input("Select a card [none when finished]: ")
            if selected.strip() == '':
                input_err = False
                selecting = False
            elif selected.upper() in deck:
                input_err = False
                deck.remove(selected.upper())
                hand.append(selected.upper())
                selected_count += 1
                show(hand)
            else:
                sound_bell()
                print "You did not enter a valid deck card. Try again. "
    return deck, hand
